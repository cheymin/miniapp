...
export default {
  ...
  methods: {
    ...
    async loadHtml() {
      ...
      try {
        ...
        if (this.debug) {
          ...
          console.log('[HtmlView] External CSS links count:', externalCSS.length);
    ...
    filterCSSDeclarations(decls) {
      const forbidden = ['visibility','opacity','zIndex','position','float','clear','overflow','top','right','bottom','left','transform','transition','animation'];
      const safe = {};
    ...
    renderNode(node, h, parentForm) {
      ...
      if (node.tag === 'input') {
        ...
        if (type === 'submit' || type === 'button' || type === 'reset') {
          ...
          return h('div', {
            style: { ...this.inlineStyle(style), padding: '8px 16px', backgroundColor: '#ddd', justifyContent: 'center', alignItems: 'center', borderRadius: '4px', margin: '4px 0' },
            on: { click: () => { if (parentForm) this.onFormSubmit(parentForm); } }
          }, [h('text', { style: { color: '#000' } }, text)]);
        }
        return h('input', { attrs: { type: type, value: attrs.value || '', placeholder: attrs.placeholder || '' }, style: this.inlineStyle(style) });
      }
      ...
      const inlineTags = ['span','a','strong','b','em','i','u','font','code','sub','sup','label','small','abbr','acronym','bdi','bdo','cite','del','dfn','ins','kbd','mark','q','s','samp','var'];
      const blockTags = ['div','p','h1','h2','h3','h4','h5','h6','ul','ol','li','header','footer','section','article','blockquote','form','nav','main','aside','figure','figcaption','details','summary','dialog','pre','dl','dt','dd','table','tr','td','th','tbody','thead','tfoot','caption','colgroup'];
      ...
      if (inlineTags.includes(node.tag)) {
        ...
        if (node.tag === 'a' && node.attrs && node.attrs.href) {
          return h('text', { style: { ...this.inlineStyle(combinedStyle), color:'#0000EE', textDecoration:'underline' }, on: { click: () => this.onLinkClick(node.attrs.href) } }, text);
        }
    ...
    shouldUseRowLayout(children) {
      ...
      return children.every(child => {
        ...
        const inlineTags = ['span','a','strong','b','em','i','u','font','code','sub','sup','img','br','label','small','abbr','acronym','bdi','bdo','cite','del','dfn','ins','kbd','mark','q','s','samp','var'];
    ...
    hasImageOrBlock(node) {
      ...
      const block = ['div','p','h1','h2','h3','h4','h5','h6','ul','ol','li','header','footer','section','article','blockquote','form','nav','main','aside','figure','input','textarea','button','select','table','tr','td','th','pre','dl','dt','dd','details','summary','figcaption'];
    ...
    inlineStyle(styleObj) {
      const allowed = ['color','fontSize','fontWeight','fontStyle','lineHeight','backgroundColor','textDecoration','width','height','margin','marginTop','marginBottom','marginLeft','marginRight','padding','paddingTop','paddingBottom','paddingLeft','paddingRight','textAlign','border','borderRadius','verticalAlign','position','whiteSpace','fontFamily'];
      const css = {};
    ...
    async executeScripts() {
      ...
      const extendNode = (node) => {
        ...
        node._extended = true;
        node.appendChild = function(child) { if (!this.children) this.children = []; this.children.push(child); };
        node.setAttribute = function(name, value) { if (!this.attrs) this.attrs = {}; this.attrs[name] = value; };
        node.removeAttribute = function(name) { if (this.attrs) delete this.attrs[name]; };
        Object.defineProperty(node, 'innerHTML', {
          get() { return that.getInnerText(this); },
          set(val) { const parsed = that.parseHTML(val); this.children = parsed.nodeTree || []; }
        });
          ...
          get() {
            if (!this._styleProxy) {
              this._styleProxy = new Proxy(this.styles || {}, {
                ...
                set(target, prop, value) { if (!this.styles) this.styles = {}; target[prop] = value; return true; }
              });
            }
      ...
      const extendTree = (tree) => {
        ...
        else { extendNode(tree); if (tree.children) extendTree(tree.children); }
      };
      this._findNodeById = (tree, id) => {
        if (!tree) return null;
        if (Array.isArray(tree)) {
          for (let n of tree) { const found = this._findNodeById(n, id); if (found) return found; }
          return null;
        } else {
    ...
    runDiagnostics() {
      console.log('[HtmlView] ========== Diagnostics Report ==========');
      ...
      const interestTags = ['a','img','input','button','textarea','select','div','span','p','h1','h2','h3','h4','h5','h6','ul','ol','li','table','tr','td','th','form','pre','code'];
